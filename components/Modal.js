		...
		<Backdrop onClick={handleClose}>
			{type === 'dropIn' && (
				<motion.div
					...
					className="rounded-xl flex flex-col justify-center bg-[#1D2226] w-full max-w-lg md:-mt-96 mx-6"
					variants={dropIn}
					initial="hidden"
					animate="visible"
					exit="exit"
				>
					<div className="text-white/75 flex items-center justify-between border-b border-white/75 px-4 py-2.5">
						<h4 className="text-xl">Create a post</h4>
						<IconButton onClick={handleClose}>
							<CloseRoundedIcon className="h-7 w-7 text-white/75" />
						</IconButton>
					</div>

					<div className="p-4 space-y-2">
						<div className="text-teal-700 flex items-center space-x-2">
